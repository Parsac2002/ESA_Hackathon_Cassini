...
*/
// link para allthingstalk del repo para este codigo: https://github.com/allthingstalk/cassini-hackathons
#include <Wire.h>
...
byte readFlameDetector(){
  byte value = digitalRead(LIGHT_SENSOR_PIN);
  return map(value, 0, 300, 0, 255);
...
void readAndSendData() {
  ...
  byte flame = readFlameDetector(); // presencia o no de llama, 0 si sí o 1 sino.
  ...
  Serial.print(" size -> ");
  Serial.print(sizeof(temperatureRaw));
  Serial.println(F("°C"));
  Serial.print(F("DHT20 - Humidity: "));
  Serial.print(humidity);
  Serial.print(" size -> ");
  Serial.print(sizeof(humidity));
  Serial.println(F("%"));
  Serial.print(F("BMP280 - Pressure: "));
  Serial.print(pressure);
  Serial.print(" size -> ");
  Serial.print(sizeof(pressure));
  Serial.println(F(" mbar"));
  Serial.print(F("Light Sensor - Value: "));
  Serial.print(light);
  Serial.print(" size -> ");
  Serial.print(sizeof(light));
  Serial.println(F("/255"));
  Serial.print(F("Sound Sensor - Value: "));
  Serial.print(sound);
  Serial.print(" size -> ");
  Serial.print(sizeof(sound));
  Serial.println(F("/255"));
  Serial.print(F("Potentiometer - Value: "));
  Serial.print(potentiometer);
  Serial.print(" size -> ");
  Serial.print(sizeof(potentiometer));
  Serial.println(F("/255"));
  
  Serial.print(F("Flame_sensor - Value: "));
  Serial.print(flame);
  Serial.print(sizeof(flame));
  Serial.println(F("/255"));
  ...
  Serial.print("X: "); Serial.print(event.acceleration.x); Serial.print(" m/s² -> size: "); Serial.print(sizeof(event.acceleration.x));
  Serial.println();
  
  Serial.print("Y: "); Serial.print(event.acceleration.y); Serial.print(" m/s² -> size: ");Serial.print(sizeof(event.acceleration.y));
  Serial.println();
  
  Serial.print("Z: "); Serial.print(event.acceleration.z); Serial.print(" m/s² -> size: ");Serial.print(sizeof(event.acceleration.z));
  Serial.println();
  ...
  if (kim1initialized) {
    ...
    delay(200);
    kineis.KIM_sendATCommandSet(TX, sizeof(TX), hexPayload, sizeof(hexPayload) - 1);// Tra
...
void initPeripherals() {
  ...
  //Initialize ADXL345 3-Axis accelerator sensor
  if (!accel.begin()) {
...
bool initKineis() {
  ...
  kineis.KIM_sendATCommandSet(AFMT, sizeof(AFMT), "1,16,32", sizeof("1,16,32") ); //formato de trama pag 14: formato 
  /*
  1 (message format) =  Kinéis standard format 
  16 (length of the CRC) =  CRC-16/XMODEM (applicable for Kinéis standard format)
  32 (length of the BCH) = BCH (applicable for Kinéis standard format)
  So 2 options: 
  - 0,0,0 for Raw format (default)
  - 1,16,32 for Kinéis standard message
  ...
  */
  ...
  /* Save configuration */
  kineis.KIM_sendATCommandSet(SAVE, sizeof(SAVE), "", sizeof("") ); //mensaje SAVE_CFG
